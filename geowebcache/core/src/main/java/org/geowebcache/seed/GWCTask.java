...
public abstract class GWCTask {
     ...
     * KILLED: task was stopped intentionally with no intent to restart.
   ...
   protected ConcurrentLinkedQueue<JobLogObject> newLogs = new ConcurrentLinkedQueue<JobLogObject>();
    ...
    public final void doAction() throws GeoWebCacheException, InterruptedException {
        ...
        try {
            doActionInternal();
                ...
                log.warn("Task " + taskId + " has been interrupted.");
        	doAbnormalExit(ie);
        	throw ie;
    ...
    protected void checkInterrupted() throws InterruptedException {
        if (Thread.interrupted()) {
            log.info("Task " + taskId + " has been flagged as interrupted.");
            if(this.state == STATE.READY || this.state == STATE.RUNNING) {
            	this.state = STATE.INTERRUPTED;
            } else {
                this.state = STATE.DEAD;
            }
            addLog(JobLogObject.createInfoLog(jobId, "Interrupted", 
            		"A thread of the job has been interrupted."));
