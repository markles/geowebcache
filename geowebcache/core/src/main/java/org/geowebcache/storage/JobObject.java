...
public class JobObject {
    ...
    /*
     * These are stored as longs for persistence purposes.
     */
    ...
    public static JobObject createJobObject(TileLayer tl, SeedRequest sr) throws GeoWebCacheException {
        ...
        if(sr.getBounds() == null) {
            bb = gridSubset.getGridSetBounds();
        } else {
            bb = sr.getBounds();
        }
        obj.bounds = bb;
...
//        obj.maxy = job.maxy;
    ...
    public void update(TileBreeder tb) {
        ...
        Iterator<GWCTask> iter = tb.getRunningTasks();
...
//        }
    ...
    public boolean equals(Object obj) {
        if(obj instanceof JobObject) {
            JobObject other = (JobObject) obj;
            if(this.srs == null && other.srs != null) {
                return false;
            } else if(this.srs != null && other.srs == null) {
                return false;
            } else if(this.srs != null && this.srs.getNumber() != other.srs.getNumber()) {
                return false;
            }
            boolean result = this.jobId == other.jobId &&
                    Objects.equal(this.layerName, other.layerName) &&
                    Objects.equal(this.state, other.state) &&
                    Objects.equal(this.gridSetId, other.gridSetId) &&
                    this.threadCount == other.threadCount &&
                    this.zoomStart == other.zoomStart &&
                    this.zoomStop == other.zoomStop &&
                    Objects.equal(this.format, other.format) &&
                    Objects.equal(this.jobType, other.jobType) &&
                    this.maxThroughput == other.maxThroughput &&
                    Objects.equal(this.priority, other.priority) &&
                    Objects.equal(this.schedule, other.schedule) &&
                    this.runOnce == other.runOnce &&
                    this.filterUpdate == other.filterUpdate &&
                    Objects.equal(this.encodedParameters, other.encodedParameters) &&
                    Objects.equal(this.bounds, other.bounds);
            return result;
        }
        return false;
    }
    ...
    @Override
    public int hashCode() {
        return Objects.hashCode(jobId, layerName, state, gridSetId, threadCount, zoomStart,
                zoomStop, format, jobType, maxThroughput, priority,
                schedule, runOnce, filterUpdate, encodedParameters, bounds, srs);
    }
}
